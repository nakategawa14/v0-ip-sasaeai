...
import { fetchBidirectionalBlockedPeerIds } from "@/lib/blocks/fetchBlockedPeerIds"
...
export default function ChatConversationScreen() {
  ...
  const bootstrap = useCallback(async () => {
    ...
    // targetId は peerId / chat_room_id のどちらでも受け取れる
    let peerId = String(targetId)
    let roomFromTarget: string | null = null
    const { data: roomCandidate } = await supabase
      .from(TABLES.CHAT_ROOMS)
      .select("id, match_id")
      .eq("id", targetId)
      .maybeSingle()
    if (roomCandidate?.id && roomCandidate.match_id) {
      roomFromTarget = String(roomCandidate.id)
      const { data: matchRow } = await supabase
        .from(TABLES.MATCHES)
        .select("user1_id, user2_id")
        .eq("id", roomCandidate.match_id)
        .maybeSingle()
      if (matchRow?.user1_id && matchRow?.user2_id) {
        peerId = matchRow.user1_id === myId ? String(matchRow.user2_id) : String(matchRow.user1_id)
      }
    }
    if (peerId === myId) {
      setLoading(false)
    ...
    const { blocked: blockedPeers, error: blockErr } = await fetchBidirectionalBlockedPeerIds(supabase, myId)
    if (blockErr) {
      setLoading(false)
      Alert.alert("読み込みエラー", blockErr, [{ text: "OK", onPress: () => router.back() }])
      return
    }
    if (blockedPeers.has(String(peerId))) {
      setLoading(false)
      Alert.alert(
        "表示できません",
        "ブロック中、または相手からブロックされているためメッセージを開けません。",
    ...
    const rid = roomFromTarget
    const rpcRes = rid
      ? { data: rid, error: null as { message?: string } | null }
      : await supabase.rpc("sasaeai_ensure_chat_room_for_peer", { p_peer_id: peerId })
    ...
    setLoading(false)
  }, [myId, targetId, router, markAsRead])
  ...
  const headerOptions = useMemo(
    () => ({
      ...
      headerLeft: () => <StackBackButton fallbackHref="/(tabs)/messages" />,
    }),
    [peerNickname],
  )
  ...
  if (!targetId) {
    return (
      <SafeAreaView style={styles.page} edges={["bottom"]}>
        <Stack.Screen options={{ title: "メッセージ", headerLeft: () => <StackBackButton fallbackHref="/(tabs)/messages" /> }} />
