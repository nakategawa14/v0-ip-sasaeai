...
import { fetchBlockedPeerIdsWhereViewerIsBlocker } from "@/lib/blocks/fetchBlockedPeerIds"
...
export default function MatchListScreen() {
  ...
  const loadProfiles = useCallback(async () => {
    ...
    if (meErr || !me) {
      setError("プロフィールが見つかりません。「プロフィールを編集」から登録を完了してください。")
      setProfiles([])
      setVerified(null)
      return
    }
    ...
    const { blocked, error: blockErr } = await fetchBlockedPeerIdsWhereViewerIsBlocker(supabase, me.id)
    if (blockErr) {
      setError(blockErr)
      setProfiles([])
      return
    ...
    const { data: rows, error: qErr } = await supabase
      .from(TABLES.PROFILES)
      .select(
        "id,nickname,gender,birth_date,prefecture,city,is_active,is_verified,is_secret_mode,profile_images,profile_image_url,created_at,updated_at",
      )
      .neq("id", me.id)
    ...
    const secretIds = list.filter((p) => p.is_secret_mode === true).map((p) => p.id)
    if (secretIds.length > 0) {
      const { data: likesReveal, error: likeErr } = await supabase
        .from(TABLES.LIKES)
        .select("from_user_id")
        .eq("to_user_id", me.id)
        .in("from_user_id", secretIds)
      if (likeErr) {
        setError(likeErr.message)
        setProfiles([])
        return
      }
      const revealed = new Set((likesReveal ?? []).map((r) => String((r as { from_user_id: string }).from_user_id)))
      list = list.filter((p) => p.is_secret_mode !== true || revealed.has(p.id))
    }
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.header}>
            <Text style={styles.subtitle}>お相手を見つけましょう。</Text>
              ...
              <View style={styles.verifyBanner}>
                ...
                <View style={styles.verifyTextWrap}>
                  ...
                  <Text style={styles.verifyBody}>
                    本人確認が済むと、マッチングや機能のご利用がスムーズになります。撮影・申請はマイページの本人確認より行えます。
                  </Text>
                </View>
              </View>
            ) : null}
