...
export default function MessagesTabScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const { data: matches, error: mErr } = await fetchMatchesParticipatedByUser(
      supabase,
      user.id,
      "id, user1_id, user2_id, last_message_at, is_active, status",
    )
    ...
    let matchRows = ((matches ?? []) as MatchRow[]).filter(
      (m) => m.is_active !== false && isMatchedRow(m.status),
    )

    const { blocked: blockedPeerIds, error: blockErr } = await fetchBidirectionalBlockedPeerIds(supabase, user.id)
    if (blockErr) {
      setError(blockErr)
      setThreads([])
      setLoading(false)
    ...
    matchRows = matchRows.filter((m) => !blockedPeerIds.has(peerIdFromMatch(m, user.id)))
  ...
  const openChat = (peerId: string) => {
    router.push({ pathname: "/(tabs)/chat/[id]", params: { id: peerId } } as Href)
  }
