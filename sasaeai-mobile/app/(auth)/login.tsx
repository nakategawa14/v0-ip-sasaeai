...
export default function LoginScreen() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (signInError) {
        // 本番は固定文言。開発時は Supabase の message を出して URL/キー不一致などを切り分けしやすくする。
        if (__DEV__) {
          setError(
            `[開発表示] ${signInError.message}${signInError.status != null ? ` (HTTP ${signInError.status})` : ""}\n\n一般的な失敗時は下記も表示されます。\nメールアドレスまたはパスワードが正しくありません`,
          )
        } else {
          ...
          <View style={styles.card}>
            ...
            <Pressable
              onPress={() =>
                Alert.alert(
                  "パスワードをお忘れですか？",
                  "パスワード再設定は Web 版の該当ページから行えます（アプリからの対応は今後追加予定です）。",
                )
              }
