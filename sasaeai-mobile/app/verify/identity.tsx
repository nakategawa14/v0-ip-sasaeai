    ...
    <ScrollView style={styles.selectScroll} contentContainerStyle={styles.selectContent} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.mynLegal}>
        ...
        <Text style={styles.mynLegalText}>
          マイナンバーカード利用時は、個人番号の取り扱い・撮影範囲について最新の法令・総務省ガイドラインに従ってください。本画面は開発用の骨組みです。
        </Text>
      </View>
    ...
    return (
      <View style={[styles.center, styles.analyzeBg]}>
        ...
        <Text style={styles.analyzeSub}>
          {documentKind
            ? `${DOCUMENT_META[documentKind].labelJa}向けパイプラインで画像を最適化し、年齢・本人照合をシミュレーションしています`
            : "画像を最適化し、年齢・本人照合をシミュレーションしています"}
        </Text>
      ...
      <View style={[styles.resultWrap, { paddingTop: insets.top + 12 }]}>
        ...
        <View style={styles.resultCard}>
          ...
          <Text style={styles.resultNote}>
            管理人に通知しました。確認までお待ちください。{"\n"}
            ※ 実運用では管理者の最終確認や、外部 API の結果を組み合わせます。DB の verification_status を pending に更新しました（カラムがある場合）。
          </Text>
        </View>
