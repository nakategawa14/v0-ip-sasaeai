-- プッシュ通知購読情報テーブルを作成（修正版）

-- テーブル作成（存在しない場合のみ）
CREATE TABLE IF NOT EXISTS sasaeai_push_subscriptions (
...
DROP POLICY IF EXISTS "Service role can view all subscriptions" ON sasaeai_push_subscriptions;
...
-- インデックス（存在しない場合のみ）
CREATE INDEX IF NOT EXISTS idx_push_subscriptions_user_id ON sasaeai_push_subscriptions(user_id);
CREATE INDEX IF NOT EXISTS idx_push_subscriptions_endpoint ON sasaeai_push_subscriptions(endpoint);
